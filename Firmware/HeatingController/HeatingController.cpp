...
#include "HeatingControllerRevA.h"
#define BAUD 9600
...
int main(void)
{
	...
	bool id2button = false;
			...
			unsigned char changestate[1] ={(unsigned char) 0x43};
	while(1)
	{
		printf("CONNECTING\n");
		pendingLed(HIGH);
...
void setup(void)
{
	...
	app.setCallback(callback); // give the MQTT layer the function we will use to process entries.
	//setup outputs
